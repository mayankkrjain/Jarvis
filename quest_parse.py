...
import wordnet
def chat_bot(tag_line,entity_list,domain_list,syn_list,data,root):
	...
	for words in tag_line:
		word_list=[]
		word_list=difflib.get_close_matches(words[0], entity_list,1,0.8)
	...
	clause=' '.join(clause)			
	if len(clause) !=0:
		(answer,root)=bs.match_attributes(data,clause,entity_list,domain_list,syn_list,root)
		return (answer,root)
